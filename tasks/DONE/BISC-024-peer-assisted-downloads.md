...
- [x] Download works when original sender is the only source — integration test (3 peers: A shares, B downloads from A)
- [x] Download works when original sender is offline but another peer has the file — integration test (A shares to B, A disconnects, C downloads from B)
- [x] Chunks are downloaded from multiple peers in parallel when available — integration test (A and B both have the file, C downloads chunks from both)
- [x] Download resumes if a peer disconnects mid-transfer — integration test
- [x] `ChunkServer` only serves complete, verified chunks — unit test
