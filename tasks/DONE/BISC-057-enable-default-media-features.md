...
- [x] `default = ["audio", "video", "video-codec", "screen-capture"]` in `bisc-app/Cargo.toml`
- [x] `cargo build -p bisc-app` compiles with all features enabled
- [x] `cargo build -p bisc-app --no-default-features` compiles without features
